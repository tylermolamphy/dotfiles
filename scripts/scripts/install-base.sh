...
if [ -n "$(command -v dnf)" ]; then
    sudo dnf update -y
    sudo dnf install stow zsh tmux vim keychain curl wget iftop inetutils-tools mosh -y
    exit 0
fi
if [ -n "$(command -v yum)" ]; then
    sudo yum update -y
    sudo yum install stow zsh tmux vim keychain curl wget iftop inetutils-tools mosh -y
    exit 0
fi
